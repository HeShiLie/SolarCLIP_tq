...
def parse_args():
    ...
    parser.add_argument('--early_stop', type=int, default=10, help='Early stop for training')
    parser.add_argument('--inner_loss_rate', type=float, default=0)
    parser.add_argument('--device', type=str,
                        default='cuda:0', help='Device for training')
    parser.add_argument("--checkpoint_path", type=str,
                        default="/mnt/nas/home/huxing/202407/ctf/SolarCLIP/checkpoints/SolarCLIP/", help="The output path to save the model.")

    # DataLoader parameters
    parser.add_argument('--num_workers', type=int, default=0,
                        help='Number of data loading workers')
    parser.add_argument('--machine', type=str, default='A40',
    ...
    parser.add_argument('--norm_type', type=str, 
                        default='ln', help='Normalization type for CLIP model')
    ...
    parser.add_argument('--weight_type_list', type=list, nargs="+", default=['cv-rdbu', '3sgm-continous'], help='Weight type list for training')
...
def main():
    ...
    checkpoint_path += args.norm_type + '/'
    ...
    if args.optimizer == 'AdamW':
        ...
        optimizer = torch.optim.SGD(
            SolarModel.parameters(), lr=args.learning_rate, weight_decay=1e-4, momentum=0.9)
    else:
    ...
    for epoch in range(epochs):
        ...
        print(f"Training epoch {epoch+1:>6}: Loss = {loss.item():<7.8f} | Loss_inner = {loss_inner.item():<7.8f} | acc = {np.mean(acc_results):<7.2%} | Lr rate = {scheduler.get_last_lr()[0]:<7.8f} | Time = {(time.time()-start_time)/60:.2f} min")
        start_time = time.time()
        ...
        if (epoch+1) % test_epoch == 0:
            with torch.no_grad():
                ...
                result_txt = f'Testing epoch {epoch+1:>6}/{epochs:<6} | '
                ...
                if logger_val_acc[-1] > max_acc_val:
                    stop_count = 0
                    max_acc_val = logger_val_acc[-1]
                    ...
                    if stop_count > args.early_stop:
                        print(f'Early stop at epoch {epoch+1}, best acc {max_acc_val:<7.2%}')
                        break
