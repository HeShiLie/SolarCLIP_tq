...
def parse_args():
    ...
    parser.add_argument('--norm_type', type=str, 
                        default='ln', help='Normalization type for the model')
...
def main(args, vae_args):
    ...
    if rank == 0:
        ...
        checkpoint_path = f"{checkpoint_path}/" + 'use_vae_lambda_1e-1_' + args.norm_type
    ...
    model = get_DiT_model_from_args(args)
    ...
    print(f"[Rank {rank}] | DiT Parameters: {sum(p.numel() for p in model.parameters()):,}")
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.decoder_modal == 'magnet-magnet':
                ...
                model.module.y_vit.load_state_dict(visual_mag_state_dict)
                requires_grad(model.module.y_embedder, False)
                y = model.module.y_embedder(x)
                ...
                model.module.y_vit.load_state_dict(visual_0094_state_dict)
                requires_grad(model.module.y_embedder, False)
                y = model.module.y_embedder(x)
