...
from Model.SolarReconModels.Pos_CLIP.SolarReconModel_Pos_CLIP_transformerDecoder import get_SolarReconModel_Pos_CLIP_transformerDecoder_from_args
...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int,
                        default=96, help='Batch size for training')
    ...
    parser.add_argument('--early_stop', type=int, 
                        default=1000, help='Early stop for training')
    parser.add_argument('--device', type=str,
                        default='cuda:1', help='Device for training')
    parser.add_argument('--embed_state_root', type=str, 
                        default='checkpoints/recon/0816_test1/SolarCLIP', help='Embedding state root path')
    parser.add_argument('--clip_model_id', type=int, 
                        default= 200, help='select the model id for the first modal')
    parser.add_argument('--vit_pretrain', type=bool,
                        default=False, help='Pretrain the vit model')
    ...
    parser.add_argument('--patch_size', type=int, 
                        default=64,help='Patch size')
    parser.add_argument('--width', type=int, 
                        default=768,help='Width of the model')
    parser.add_argument('--vit_layers', type=int, 
                        default=12,help='Number of vit layers')
    parser.add_argument('--transformer_layers', type=int, default=2,
                        help='Number of layers in ordinary transformer')
    parser.add_argument('--hidden_dim', type=int, 
                        default=768,help='Hidden dimension')
    parser.add_argument('--norm_type', type=str, 
                        default='bn1d',help='Normalization type')
    parser.add_argument('--output_dim', type=int, 
                        default=1,help='Output dimension')
    parser.add_argument('--output_size', type=int, 
                        default=1024,help='Output size')
    parser.add_argument('--deprojection_type', type=str, 
                        default='linear',help='Deprojection type')
    parser.add_argument('--with_bias', type=bool, 
                        default=False,help='With bias')
    
    # Modal parameters
    parser.add_argument('--decoder_modal', type=str, 
                        default='magnet-0094', help='Decoder modal for training')
    ...
    parser.add_argument('--weight_type_list', type=str, nargs="+",
                        default=['cv-rdbu', '3sgm-continous'], help='Weights type list for training')
    parser.add_argument('--weight_or_not', type=bool,
                        default=True, help='Use weight or not')
    parser.add_argument('--abs', type=bool, 
                        default=False, help='Use abs for loss')

    # Save parameters
    parser.add_argument("--checkpoint_path", type=str,
                        default="/mnt/nas/home/huxing/202407/ctf/SolarCLIP_tq/checkpoints/recon/TransformerDecoder/", help="The output path to save the model.")
...
def train(args, train_loader, val_loader):
    ...
    SolarReconModel = get_SolarReconModel_Pos_CLIP_transformerDecoder_from_args(args).to(device)
    if args.vit_pretrain:
    ...
    iter_time = time.time()
    print(f'Start training SolarReconModel using LinearDecoder')
    ...
    for epoch in range(epochs):
        ...
        for i, data in enumerate(train_loader):
            ...
            if args.decoder_modal == 'magnet-0094':
                ...
                weights, _ = get_weights(args.weight_type_list[1], modal_2)
            ...
            mse_weight, mse = SolarReconModel.calculate_loss(modal, modal_2, weights, args.abs)
            iteration_txt += f" Forward time: {(time.time()-epoch_time)/60:.2f} min | iteration weight mse {mse_weight.item():<10.8f} | iteration mse {mse.item():<10.8f} |"
            epoch_time = time.time()

            optimizer.zero_grad()
            if args.weight_or_not:
                mse_weight.backward()
            else:
                mse.backward()
            optimizer.step()
            ...
            train_weight_mse.append(mse_weight.item())
        ...
        scheduler.step()
        print(f'Epoch {epoch+1:>6}/{args.epochs:<6} | Train mean weight mse {np.mean(train_weight_mse):<10.8f} | Train mean mse {np.mean(train_mse):<10.8f} | LR {logger_lr[-1]:<10.8f} | Cost {(time.time()-start_time)/60:.2f} min')
        ...
        if (epoch+1) % test_epoch == 0: 
            with torch.no_grad():
                ...
                for i, data in enumerate(val_loader):
                    ...
                    if first_batch:
                        recon_image= SolarReconModel(modal).cpu().numpy()
                        modal = modal.cpu().numpy()
                        modal_2 = modal_2.cpu().numpy()
                        if args.decoder_modal == 'magnet-0094':
                            mag_image = modal
                            h_image = modal_2
                        else:
...
def main():
    ...
    train_loader = Solardataloader_subset.get_loader_by_time(time_step=60*12*20,time_interval=[
                                                             start_date, end_date], modal_list=args.modal_list, load_imgs= True, enhance_list=args.image_preprocess, batch_size=args.batch_size, shuffle=True, num_workers=args.num_workers, sampler='None')
    start_date = transfer_date_to_id(2020, 6, 30)
    end_date = transfer_date_to_id(2024, 6, 30)
    val_loader = Solardataloader_subset.get_loader_by_time(time_step=60*12*20,time_interval=[
                                                           start_date, end_date], modal_list=args.modal_list,  load_imgs= True, enhance_list=[args.image_preprocess[0],0,0], batch_size=args.batch_size, shuffle=False, num_workers=args.num_workers, sampler='None')
