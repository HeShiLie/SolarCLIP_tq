...
class SolarReconModel_VAE_without_CLIP(nn.Module):
    def __init__(self, 
                 input_size: int = 1024, 
        ...
        #     nn.ELU(),
        #     VAE_ResidualBlock(hidden_dim, hidden_dim*2, kernel_size=7, stride=4, padding=3),  # B, 128, 1024, 1024 -> B, 256, 256, 256
        #     VAE_ResidualBlock(hidden_dim*2, hidden_dim*4, kernel_size=7, stride=4, padding=3),  # B, 256, 256, 256 -> B, 512, 64, 64
        #     VAE_ResidualBlock(hidden_dim*4, hidden_dim*8, kernel_size=7, stride=4, padding=3),  # B, 512, 64, 64 -> B, 1024, 16, 16
        #     nn.GroupNorm(group_nums, hidden_dim*8), # B, 1024, 16, 16 -> B, 1024, 16, 16
        #     nn.ELU(),
        #     nn.Conv2d(hidden_dim*8, self.latent_dim*2, kernel_size=1, stride=1, padding=0), # B, 1024, 16, 16 -> B, 6, 16, 16
        # )
        ...
        #     nn.ConvTranspose2d(self.latent_dim, hidden_dim*8, kernel_size=3, stride=1, padding=1), # B, 3, 128, 128 -> B, 1024, 16, 16
        #     nn.ELU(),
        #     nn.Upsample(scale_factor=4, mode='nearest'), # B, 1024, 16, 16 -> B, 1024, 64, 64
        #     VAE_ResidualBlock(hidden_dim*8, hidden_dim*4, kernel_size=3, stride=1, padding=1), # B, 1024, 64, 64 -> B, 512, 64, 64
        #     nn.Upsample(scale_factor=4, mode='nearest'), # B, 512, 64, 64 -> B, 512, 256, 256
        #     VAE_ResidualBlock(hidden_dim*4, hidden_dim*2, kernel_size=3, stride=1, padding=1), # B, 512, 256, 256 -> B, 256, 256, 256
        #     nn.Upsample(scale_factor=4, mode='nearest'), # B, 128, 512, 512 -> B, 128, 1024, 1024
...
def get_SolarReconModel_VAE_without_CLIP_from_args(args):
    return SolarReconModel_VAE_without_CLIP(
