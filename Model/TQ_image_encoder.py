...
class patch_norm(nn.Module):
    def __init__(self, norm_type = 'bn1d', d_model = 512, eps = 1e-5):
        super().__init__()
...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, norm_type = 'bn1d', attn_mask: torch.Tensor = None):
        super().__init__()

        self.attn = nn.MultiheadAttention(d_model, n_head, batch_first=True)
        self.ln_1 = patch_norm(norm_type, d_model)
        ...
        self.ln_2 = patch_norm(norm_type, d_model)
...
class Transformer(nn.Module):
   def __init__(self, width: int, layers: int, heads: int, drop_out: float, attn_mask: torch.tensor = None):
      ...
      self.width = width
      self.layers = layers
      self.resblocks = nn.Sequential(*[ResidualAttentionBlock(d_model=width, n_head=heads,norm_type='bn1d', attn_mask=attn_mask) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
 def __init__(self, in_channels,input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,
    ...
    self.ln_post = patch_norm('bn_1d', width)
