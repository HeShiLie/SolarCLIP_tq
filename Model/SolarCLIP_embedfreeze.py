...
class SolarCLIP_MODEL(nn.Module):
    def __init__(self,
                 ...
                 hidden_dropout_prob_embeddings_mag: float,
                 vision_layers_mag: Union[Tuple[int, int, int, int], int],
                 hidden_dropout_prob_transformer_mag: float,
                 # spectral vision
                 image_resolution_H: int,
                 vision_patch_size_H: int,
                 hidden_dropout_prob_embeddings_H: float,
                 vision_layers_H: Union[Tuple[int, int, int, int], int],
                 hidden_dropout_prob_transformer_H: float,
                 #vision_width: int,
                 # loss token type
        ...
        self.visual_mag = ViTransformer(
            # Embeddings
                input_resolution=image_resolution_mag,
                patch_size=vision_patch_size_mag,
                in_channels=1,
                hidden_dropout_prob_embeddings= hidden_dropout_prob_embeddings_mag,
                width=vision_width, #768
            # Transformer
                layers=vision_layers_mag,
                heads=vision_heads,
                hidden_dropout_prob_transformer= hidden_dropout_prob_transformer_mag,
                token_type = transformer_token_type
            )
        
        self.visual_H =  ViTransformer(
            # Embeddings
                input_resolution=image_resolution_H,
                patch_size=vision_patch_size_H,
                in_channels=1,
                hidden_dropout_prob_embeddings= hidden_dropout_prob_embeddings_H,
                width=vision_width, #768
            # Transformer
                layers=vision_layers_H,
                heads=vision_heads,
                hidden_dropout_prob_transformer= hidden_dropout_prob_transformer_H,
                token_type = transformer_token_type
            )
    ...
    @property
    def dtype(self):
        return self.visual_mag.Embeddings.conv1.weight.dtype
    ...
    def forward(self, image_mag, image_H, token_weight_1_type='cv-rdbu', token_weight_2_type='3sgm-continous'):
        ...
        # compute the weighted loss
            ...
            if token_weight_1_type is None: 
                token_weight_1 = torch.ones([B,L],dtype = mag_features.dtype,device = mag_features.device)
            else:
                # B,C,H,W  ##todo weights_computation
                _, token_weight_1 = get_weights(token_weight_1_type, image_mag)
            B = H_features.shape[0]
            L = H_features.shape[1]
            if token_weight_2_type is None:
                 token_weight_2 = torch.ones([B,L],dtype = H_features.dtype,device = H_features.device)
            else:
                _, token_weight_2 = get_weights(token_weight_2_type, image_H)
...
def get_model_from_args(args):
    return SolarCLIP_MODEL(
            ...
            hidden_dropout_prob_embeddings_mag = args.hidden_dropout_prob_embeddings_mag,
            vision_layers_mag =  args.vision_layers_mag,
            hidden_dropout_prob_transformer_mag = args.hidden_dropout_prob_transformer_mag,
            # spectral vision
            image_resolution_H = args.image_resolution_H,
            vision_patch_size_H = args.vision_patch_size_H,
            hidden_dropout_prob_embeddings_H = args.hidden_dropout_prob_embeddings_H,
            vision_layers_H = args.vision_layers_H,
            hidden_dropout_prob_transformer_H = args.hidden_dropout_prob_transformer_H,
            #vision_width =int,
            # loss token type
